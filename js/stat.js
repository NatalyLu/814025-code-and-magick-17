...
var renderText = function (ctx, color) {
  ...
  ctx.fillStyle = color;
  ctx.fillText('Ура Вы победили!', CLOUD_X + 2 * GAP, CLOUD_HEIGHT - GAP - BASE_LINE - BAR_HEIGHT - BASE_LINE - GAP - BASE_LINE);
  ctx.fillText('Список результатов:', CLOUD_X + 2 * GAP, CLOUD_HEIGHT - GAP - BASE_LINE - BAR_HEIGHT - BASE_LINE - GAP);
};
...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    ctx.fillText(names[i], CLOUD_X + BAR_SPACE + (BAR_WIDTH + BAR_SPACE) * i, CLOUD_HEIGHT - GAP);
    if (names[i] === 'Вы') {
